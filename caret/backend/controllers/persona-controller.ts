...
import { CaretPersona as Persona } from "../../types"
...
export class PersonaController extends CaretController {
	 ...
	 */
	constructor(context: vscode.ExtensionContext, postMessage: (message: ExtensionMessage) => Promise<void>) {
	 ...
	 * 퍼소나 로드를 처리합니다.
	 */
	private async handleLoadPersona(): Promise<void> {
		this.log("퍼소나 로드 요청")
		try {
	...
	private async handleAddOrUpdatePersona(message: WebviewMessage): Promise<void> {
		try {
			const persona = (message as any).persona as Persona
			if (!persona) {
