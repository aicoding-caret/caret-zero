...
import { CaretAsk, ExtensionMessage } from "@shared/ExtensionMessage"
import { ApiProvider } from "@shared/api"
import { HistoryItem } from "@shared/HistoryItem"
import { getSavedCaretMessages, getSavedApiConversationHistory } from "@core/storage/disk"
...
export function createTestServer(webviewProvider?: WebviewProvider): http.Server {
	// Try to show the Caret sidebar
	Logger.log("[createTestServer] Opening Caret in sidebar...")
		...
		req.on("end", async () => {
			try {
				...
				if (!visibleWebview || !visibleWebview.controller) {
					res.writeHead(500)
					res.end(JSON.stringify({ error: "No active Caret instance found" }))
					return
				}
				...
				try {
					...
					if (apiKey) {
						...
						const updatedConfig = {
							...apiConfiguration,
							apiProvider: "caret" as ApiProvider,
							caretApiKey: apiKey,
						}

						// Store the API key securely
						await storeSecret(visibleWebview.controller.context, "caretApiKey", apiKey)

						// Update the API configuration
						await updateApiConfiguration(visibleWebview.controller.context, updatedConfig)

						// Update global state to use caret provider
						await updateGlobalState(visibleWebview.controller.context, "apiProvider", "caret" as ApiProvider)
					...
					if (chatSettings.mode === "arch") {
						...
						await visibleWebview.controller.toggleModeWithChatSettings({ mode: "dev" })
					}
					...
					const result = await visibleWebview.controller.initCaretWithTask(task)
					...
					try {
						...
						try {
							if (typeof taskId === "string") {
								messages = await getSavedCaretMessages(visibleWebview.controller.context, taskId)
							}
						} catch (error) {
							Logger.log(`Error getting saved Caret messages: ${error}`)
						}
...
export function createMessageCatcher(webviewProvider: WebviewProvider): vscode.Disposable {
	Logger.log("Caret message catcher registered")
	...
	if (webviewProvider && webviewProvider.controller) {
		...
		webviewProvider.controller.postMessageToWebview = async (message: ExtensionMessage) => {
			...
			if (message.type === "partialMessage" && message.partialMessage?.type === "ask" && !message.partialMessage.partial) {
				const askType = message.partialMessage.ask as CaretAsk
	...
	return new vscode.Disposable(() => {
		...
		Logger.log("Caret message catcher disposed")
	})
}
 ...
 */
async function autoRespondToAsk(webviewProvider: WebviewProvider, askType: CaretAsk, askText?: string): Promise<void> {
	...
	switch (askType) {
		...
		case "new_task":
			// Decaret creating a new task to keep the current task running
			responseType = "messageResponse"
			...
			setTimeout(async () => {
				try {
					if (webviewProvider.controller) {
						...
						await webviewProvider.controller.toggleModeWithChatSettings({ mode: "dev" })
					}
				} catch (error) {
