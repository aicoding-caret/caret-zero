...
class TelemetryService {
    ...
    async updateTelemetryState(didUserOptIn) {
        ...
        if (globalTelemetryEnabled) {
            ...
            if (didUserOptIn) {
                void vscode.window
                    .showWarningMessage("Anonymous Cline error and usage reporting is enabled, but VSCode telemetry is disabled. To enable error and usage reporting for this extension, enable VSCode telemetry in settings.", "Open Settings")
                    .then((selection) => {
     ...
     */
    captureTaskCreated(taskId, apiProvider, collect = false) {
        this.capture({
     ...
     * @param collect If true, collect event instead of sending
     */
    captureTaskRestarted(taskId, apiProvider, collect = false) {
        this.capture({
     ...
     * @param collect If true, collect event instead of sending
     */
    captureTaskCompleted(taskId, collect = false) {
        this.capture({
     ...
     */
    captureConversationTurnEvent(taskId, provider = "unknown", model = "unknown", source, collect = false) {
     ...
     */
    captureTokenUsage(taskId, tokensIn, tokensOut, model, collect = false) {
        this.capture({
     ...
     */
    captureModeSwitch(taskId, mode, collect = false) {
        this.capture({
     ...
     */
    captureTaskFeedback(taskId, feedbackType, collect = false) {
     ...
     */
    captureToolUsage(taskId, tool, modelId, autoApproved, success, collect = false) {
        this.capture({
     ...
     */
    captureCheckpointUsage(taskId, action, durationMs, collect = false) {
     ...
     */
    captureDiffEditFailure(taskId, modelId, errorType, collect = false) {
        this.capture({
     ...
     */
    captureModelSelected(model, provider, taskId, collect = false) {
        this.capture({
     ...
     */
    captureHistoricalTaskLoaded(taskId, collect = false) {
        this.capture({
     ...
     */
    captureRetryClicked(taskId, collect = false) {
        this.capture({
     ...
     */
    captureBrowserToolStart(taskId, browserSettings, collect = false) {
     ...
     */
    captureBrowserToolEnd(taskId, stats, collect = false) {
        if (!this.isCategoryEnabled("browser")) {
            return;
        }
        this.capture({
     ...
     */
    captureBrowserError(taskId, errorType, errorMessage, context, collect = false) {
        if (!this.isCategoryEnabled("browser")) {
            return;
        }
        this.capture({
     ...
     */
    captureOptionSelected(taskId, qty, mode, collect = false) {
        this.capture({
     ...
     */
    captureOptionsIgnored(taskId, qty, mode, collect = false) {
        this.capture({
     ...
     * @param collect If true, collect event instead of sending
     */
    captureGeminiApiPerformance(taskId, modelId, data, collect = false) {
        this.capture({
     ...
     */
    captureModelFavoritesUsage(model, isFavorited, collect = false) {
        this.capture({
    ...
    captureButtonClick(button, taskId, collect = false) {
        this.capture({
    ...
    async sendCollectedEvents(taskId) {
        if (!this.telemetryEnabled) {
            return;
        }
