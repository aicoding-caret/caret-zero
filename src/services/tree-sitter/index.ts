...
export async function parseSourceCodeForDefinitionsTopLevel(
	...
	const allowedFilesToParse = caretIgnoreController ? caretIgnoreController.filterPaths(filesToParse) : filesToParse

	for (const filePath of allowedFilesToParse) {
		const definitions = await parseFile(filePath, languageParsers, caretIgnoreController)
		if (definitions) {
...
async function parseFile(
	...
	if (caretIgnoreController && !caretIgnoreController.validateAccess(filePath)) {
		return null
	}
