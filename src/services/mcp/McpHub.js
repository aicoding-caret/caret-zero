...
export class McpHub {
    ...
    connections = [];
    isConnecting = false;
    constructor(getMcpServersPath, getSettingsDirectoryPath, postMessageToWebview, clientVersion) {
    ...
    getMode() {
        return vscode.workspace.getConfiguration("caret.mcp").get("mode", "full");
    }
  ...
  "mcpServers": {
    ...
    async readAndValidateMcpSettingsFile() {
        try {
            ...
            try {
                ...
                vscode.window.showErrorMessage("Invalid MCP settings format. Please ensure your settings follow the correct JSON format.");
                return undefined;
            }
    ...
    async connectToServerRPC(name, config) {
