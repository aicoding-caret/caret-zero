...
export class VsCodeLmHandler implements ApiHandler, SingleCompletionHandler {
	...
	private async countTokens(text: string | VSCodeLanguageModelChatMessage): Promise<number> {
		...
		if (!this.currentRequestCancellation) {
			console.warn("Caret <Language Model API>: No cancellation token available for token counting")
			return 0
		}

		// Validate input
		if (!text) {
			console.debug("Caret <Language Model API>: Empty text provided for token counting")
			return 0
		}
		...
		try {
			...
			if (typeof text === "string") {
				...
				if (!text.content || (Array.isArray(text.content) && text.content.length === 0)) {
					console.debug("Caret <Language Model API>: Empty chat message content")
					return 0
				}
				tokenCount = await this.client.countTokens(text, this.currentRequestCancellation.token)
			} else {
				console.warn("Caret <Language Model API>: Invalid input type for token counting")
				return 0
			}

			// Validate the result
			if (typeof tokenCount !== "number") {
				console.warn("Caret <Language Model API>: Non-numeric token count received:", tokenCount)
				return 0
			}

			if (tokenCount < 0) {
				console.warn("Caret <Language Model API>: Negative token count received:", tokenCount)
				return 0
			}
			...
			if (error instanceof vscode.CancellationError) {
				console.debug("Caret <Language Model API>: Token counting cancelled by user")
				return 0
			}

			const errorMessage = error instanceof Error ? error.message : "Unknown error"
			console.warn("Caret <Language Model API>: Token counting failed:", errorMessage)
