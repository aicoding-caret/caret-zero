...
// 개별 파일 쌍 테스트
    it.each([
        ...
        console.log(`${basename}: MD=${result.mdTokens} JSON=${result.jsonTokens} 절감=${result.reduction} (${result.reductionPercent.toFixed(2)}%)`);
    ...
    it('전체 시스템 프롬프트 토큰 최적화 테스트', async () => {
        ...
        const allResults = await Promise.all(pairs.map(async basename => {
