...
export async function parseSlashCommands(
...
): Promise<{ processedText: string; needsCaretrulesFileCheck: boolean }> {
	...
	for (const { tag, regex } of tagPatterns) {
		...
		if (match) {
			...
			if (SUPPORTED_DEFAULT_COMMANDS.includes(commandName)) {
				...
				return { processedText: processedText, needsCaretrulesFileCheck: commandName === "newrule" ? true : false }
			}
			...
			if (matchingWorkflow) {
				try {
					...
					return { processedText, needsCaretrulesFileCheck: false }
				} catch (error) {
	...
	return { processedText: text, needsCaretrulesFileCheck: false }
}
