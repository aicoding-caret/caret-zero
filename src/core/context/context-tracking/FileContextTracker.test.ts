...
describe("FileContextTracker", () => {
	...
	it("should add a record when a file is read by a tool", async () => {
		...
		expect(fileEntry.caret_read_date).to.be.a("number")
		expect(fileEntry.caret_edit_date).to.be.null
	})

	it("should add a record when a file is edited by Caret", async () => {
		const filePath = "src/test-file.ts"

		await tracker.trackFileContext(filePath, "caret_edited")
		...
		expect(activeEntry.record_source).to.equal("caret_edited")
		expect(activeEntry.caret_read_date).to.be.a("number")
		expect(activeEntry.caret_edit_date).to.be.a("number")
	})
	...
	it("should mark existing entries as stale when adding a new entry for the same file", async () => {
		...
		mockTaskMetadata.files_in_context = [
			{
				...
				caret_read_date: Date.now() - 1000, // 1 second ago
				caret_edit_date: null,
				user_edit_date: null,
			},
		]
		...
		expect(newEntry.record_source).to.equal("caret_edited")
	})
	...
	it("should not track Caret edits as user edits", async () => {
		const filePath = "src/test-file.ts"

		// First track the file to set up the watcher
		await tracker.trackFileContext(filePath, "read_tool")

		// Mark the file as edited by Caret
		tracker.markFileAsEditedByCaret(filePath)
