...
import { ensureRulesDirectoryExists, ensureTaskDirectoryExists, getSavedApiConversationHistory, getSavedClineMessages, GlobalFileNames, saveApiConversationHistory, saveClineMessages, } from "@core/storage/disk";
...
import { refreshExternalRulesToggles, getLocalWindsurfRules, getLocalCursorRules, } from "@core/context/instructions/user-instructions/external-rules";
...
export class Task {
    ...
    enableCheckpoints;
    constructor(context, mcpHub, workspaceTracker, updateTaskHistory, postStateToWebview, postMessageToWebview, reinitExistingTaskFromId, cancelTask, apiConfiguration, autoApprovalSettings, browserSettings, chatSettings, shellIntegrationTimeout, terminalReuseEnabled, enableCheckpointsSetting, customInstructions, task, images, files, historyItem) {
        ...
        let effectiveApiConfiguration = {
            ...
            onRetryAttempt: (attempt, maxRetries, delay, error) => {
                ...
                if (lastApiReqStartedIndex !== -1) {
                    try {
                        const currentApiReqInfo = JSON.parse(this.clineMessages[lastApiReqStartedIndex].text || "{}");
                        ...
                        this.postStateToWebview().catch((e) => console.error("Error posting state to webview in onRetryAttempt:", e));
    ...
    async migratePreferredLanguageToolSetting() {
        const config = vscode.workspace.getConfiguration("cline");
        const preferredLanguage = config.get("preferredLanguage");
    ...
    async *attemptApiRequest(previousApiReqIndex) {
        ...
        const preferredLanguage = getLanguageKey(this.chatSettings.preferredLanguage);
        ...
        if (settingsCustomInstructions ||
            ...
            const userInstructions = addUserInstructions(settingsCustomInstructions, globalClineRulesFileInstructions, localClineRulesFileInstructions, localCursorRulesFileInstructions, localCursorRulesDirInstructions, localWindsurfRulesFileInstructions, clineIgnoreInstructions, preferredLanguageInstructions);
            systemPrompt += userInstructions;
        }
        const contextManagementMetadata = await this.contextManager.getNewContextMessagesAndMetadata(this.apiConversationHistory, this.clineMessages, this.api, this.conversationHistoryDeletedRange, previousApiReqIndex, await ensureTaskDirectoryExists(this.getContext(), this.taskId));
        ...
        try {
            ...
            if (isAnthropic && isAnthropicContextWindowError && !this.didAutomaticallyRetryFailedApiRequest) {
                this.conversationHistoryDeletedRange = this.contextManager.getNextTruncationRange(this.apiConversationHistory, this.conversationHistoryDeletedRange, "quarter");
                await this.saveClineMessagesAndUpdateHistory();
    ...
    async handleStreamingJsonReplacement(block, relPath, currentFullJson) {
        ...
        if (block.partial) {
            ...
            if (!this.streamingJsonReplacer) {
                ...
                this.streamingJsonReplacer = new StreamingJsonReplacer(this.diffViewProvider.originalContent || "", onContentUpdated, onError);
                this.lastProcessedJsonLength = 0;
            }
    ...
    async presentAssistantMessage() {
        ...
        switch (block.type) {
            ...
            case "tool_use":
                ...
                const pushToolResult = (content, isClaude4ModelFamily = false) => {
                ...
                const pushAdditionalToolFeedback = (feedback, images, fileContentString) => {
                ...
                const handleError = async (action, error, isClaude4ModelFamily = false) => {
                    if (this.abandoned) {
                    ...
                    case "list_files": {
                        ...
                        try {
                            if (block.partial) {
                                ...
                                if (!relDirPath) {
                                    this.consecutiveMistakeCount++;
                                    pushToolResult(await this.sayAndCreateMissingParamError("list_files", "path"), isClaude4ModelFamily);
                                    await this.saveCheckpoint();
                                    break;
                                }
                                this.consecutiveMistakeCount = 0;
                    ...
                    case "search_files": {
                        ...
                        try {
                            if (block.partial) {
                                ...
                                if (!relDirPath) {
                                    this.consecutiveMistakeCount++;
                                    pushToolResult(await this.sayAndCreateMissingParamError("search_files", "path"), isClaude4ModelFamily);
                                    await this.saveCheckpoint();
                                    break;
                                }
                                if (!regex) {
                                    this.consecutiveMistakeCount++;
                                    pushToolResult(await this.sayAndCreateMissingParamError("search_files", "regex"), isClaude4ModelFamily);
                                    await this.saveCheckpoint();
                                    break;
                                }
                                this.consecutiveMistakeCount = 0;
                            ...
                            if (block.partial) {
                                if (action === "launch") {
                                    if (this.shouldAutoApproveTool(block.name)) {
                                        ...
                                        await this.say("browser_action_launch", removeClosingTag("url", url), undefined, undefined, block.partial);
                                    }
                                    else {
                        ...
                        try {
                            if (block.partial) {
                                ...
                                if (text || (images && images.length > 0) || (condenseFiles && condenseFiles.length > 0)) {
                                    ...
                                    this.conversationHistoryDeletedRange = this.contextManager.getNextTruncationRange(this.apiConversationHistory, this.conversationHistoryDeletedRange, keepStrategy);
                                    await this.saveClineMessagesAndUpdateHistory();
                                    await this.contextManager.triggerApplyStandardContextTruncationNoticeChange(Date.now(), await ensureTaskDirectoryExists(this.getContext(), this.taskId));
                                }
                                await this.saveCheckpoint();
                    ...
                    case "report_bug": {
                        ...
                        try {
                            if (block.partial) {
                                ...
                                const providerAndModel = `${(await getWorkspaceState(this.getContext(), "apiProvider"))} / ${this.api.getModel().id}`;
                                ...
                                if (text || (images && images.length > 0) || (reportBugFiles && reportBugFiles.length > 0)) {
                                    ...
                                    pushToolResult(formatResponse.toolResult(`The user did not submit the bug, and provided feedback on the Github issue generated instead:\n<feedback>\n${text}\n</feedback>`, images, fileContentString));
                                }
                                else {
                                    // If no response, the user accepted the condensed version
                            ...
                            if (block.partial) {
                                if (command) {
                                    ...
                                    if (lastMessage && lastMessage.ask === "command") {
                                        ...
                                        await this.say("completion_result", removeClosingTag("result", result), undefined, undefined, false);
                                    ...
                                    await this.say("completion_result", removeClosingTag("result", result), undefined, undefined, block.partial);
                                }
                                break;
                            }
                            else {
    ...
    async recursivelyMakeClineRequests(userContent, includeFileDetails = false) {
        ...
        const currentProviderId = (await getWorkspaceState(this.getContext(), "apiProvider"));
        ...
        try {
            ...
            const updateApiReqMsg = (cancelReason, streamingFailedMessage) => {
                const currentApiReqInfo = JSON.parse(this.clineMessages[lastApiReqIndex].text || "{}");
            ...
            const abortStream = async (cancelReason, streamingFailedMessage) => {
                ...
                await this.saveClineMessagesAndUpdateHistory();
                telemetryService.captureConversationTurnEvent(this.taskId, currentProviderId, this.api.getModel().id, "assistant", true);
            ...
            return await Promise.all(userContent.map(async (block) => {
                if (block.type === "text") {
                    ...
                    if (block.text.includes("<feedback>") ||
                        ...
                        const parsedText = await parseMentions(block.text, cwd, this.urlContentFetcher, this.fileContextTracker);
