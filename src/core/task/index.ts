...
export class Task {
	...
	private caretIgnoreController: CaretIgnoreController
	private askResponse?: CaretAskResponse
	...
	private async addToCaretMessages(message: CaretMessage) {
		// these values allow us to reconstruct the conversation history at the time this caret message was created
		// it's important that apiConversationHistory is initialized before we add caret messages
		message.conversationHistoryIndex = this.apiConversationHistory.length - 1 // NOTE: this is the index of the last added message which is the user message, and once the caretmessages have been presented we update the apiconversationhistory with the completed assistant message. This means when resetting to a message, we need to +1 this index to get the correct assistant message that this tool use corresponds to
		message.conversationHistoryDeletedRange = this.conversationHistoryDeletedRange
		this.caretMessages.push(message)
		await this.saveCaretMessagesAndUpdateHistory()
	}

	private async overwriteCaretMessages(newMessages: CaretMessage[]) {
		this.caretMessages = newMessages
		await this.saveCaretMessagesAndUpdateHistory()
	}

	private async saveCaretMessagesAndUpdateHistory() {
		try {
			await saveCaretMessages(this.getContext(), this.taskId, this.caretMessages)

			// combined as they are in ChatView
			const apiMetrics = getApiMetrics(combineApiRequests(combineCommandSequences(this.caretMessages.slice(1))))
			const taskMessage = this.caretMessages[0] // first message is always the task say
			const lastRelevantMessage =
				this.caretMessages[
					findLastIndex(this.caretMessages, (m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task"))
				]
		...
		if (!didWorkspaceRestoreFail) {
			switch (restoreType) {
				...
				case "taskAndWorkspace":
					...
					const deletedMessages = this.caretMessages.slice(messageIndex + 1)
					const deletedApiReqsMetrics = getApiMetrics(combineApiRequests(combineCommandSequences(deletedMessages)))

					const newCaretMessages = this.caretMessages.slice(0, messageIndex + 1)
					await this.overwriteCaretMessages(newCaretMessages) // calls saveCaretMessages which saves historyItem
			...
			if (restoreType !== "task") {
				...
				const checkpointMessages = this.caretMessages.filter((m) => m.say === "checkpoint_created")
		...
		try {
			if (seeNewChangesSinceLastTaskCompletion) {
				...
				const lastTaskCompletedMessageCheckpointHash = findLast(
					this.caretMessages.slice(0, messageIndex),
				...
				const firstCheckpointMessageCheckpointHash = this.caretMessages.find(
	...
	async doesLatestTaskCompletionHaveNewChanges() {
		...
		// Get last task completed
		const lastTaskCompletedMessage = findLast(this.caretMessages.slice(0, messageIndex), (m) => m.say === "completion_result")
	...
	async ask(
		...
		response: CaretAskResponse
		text?: string
		images?: string[]
	}> {
		// If this Caret instance was aborted by the provider, then the only thing keeping us alive is a promise still running in the background, in which case we don't want to send its result to the webview as it is attached to a new instance of Caret now. So we can safely ignore the result of any active promises, and this class will be deallocated. (Although we set Caret = undefined in provider, that simply removes the reference to this instance, but the instance is still alive until this promise resolves or rejects.)
		if (this.abort) {
	...
	async handleWebviewAskResponse(askResponse: CaretAskResponse, text?: string, images?: string[]) {
		this.askResponse = askResponse
		this.askResponseText = text
		this.askResponseImages = images
	}

	async say(type: CaretSay, text?: string, images?: string[], partial?: boolean): Promise<undefined> {
		if (this.abort) {
			throw new Error("Caret instance aborted")
		}
	...
	private async resumeTaskFromHistory() {
		...
		const modifiedCaretMessages = await getSavedCaretMessages(this.getContext(), this.taskId)

		// Remove any resume messages that may have been added before
		const lastRelevantMessageIndex = findLastIndex(
			modifiedCaretMessages,
			(m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task"),
		)
		if (lastRelevantMessageIndex !== -1) {
			modifiedCaretMessages.splice(lastRelevantMessageIndex + 1)
		}
		...
		if (lastApiReqStartedIndex !== -1) {
			const lastApiReqStarted = modifiedCaretMessages[lastApiReqStartedIndex]
			const { cost, cancelReason }: CaretApiReqInfo = JSON.parse(lastApiReqStarted.text || "{}")
			if (cost === undefined && cancelReason === undefined) {
				modifiedCaretMessages.splice(lastApiReqStartedIndex, 1)
			}
		}
		...
		let askType: CaretAsk
		if (lastCaretMessage?.ask === "completion_result") {
		...
		// need to make sure that the api conversation history can be resumed by the api, even if it goes out of sync with caret messages
		...
		const agoText = (() => {
			const timestamp = lastCaretMessage?.ts ?? Date.now()
			const now = Date.now()
	...
	private async initiateTaskLoop(userContent: UserContent): Promise<void> {
		...
		while (!this.abort) {
			const didEndLoop = await this.recursivelyMakeCaretRequests(nextUserContent, includeFileDetails)
			includeFileDetails = false // we only need file details the first time

			//  The way this agentic loop works is that caret will be given a task that he then calls tools to complete. unless there's an attempt_completion call, we keep responding back to him with his tool's responses until he either attempt_completion or does not use anymore tools. If he does not use anymore tools, we ask him to consider if he's completed the task and then call attempt_completion, otherwise proceed with completing the task.
			// There is a MAX_REQUESTS_PER_TASK limit to prevent infinite requests, but Caret is prompted to finish the task as efficiently as he can.
			...
			if (didEndLoop) {
				...
				// this.say(
				// 	"tool",
				// 	"Caret responded with only text blocks but has not called attempt_completion yet. Forcing him to continue with task..."
				// )
				nextUserContent = [
					{
						type: "text",
	...
	async saveCheckpoint(isAttemptCompletionMessage: boolean = false) {
		...
		this.caretMessages.forEach((message) => {
		...
		// for (let i = this.caretMessages.length - 1; i >= 0; i--) {
		// 	const message = this.caretMessages[i]
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		if (caretIgnoreContent) {
			caretIgnoreInstructions = formatResponse.caretIgnoreInstructions(caretIgnoreContent)
		}
		...
		if (contextManagementMetadata.updatedConversationHistoryDeletedRange) {
			...
			await this.saveCaretMessagesAndUpdateHistory() // saves task history item which we use to keep track of conversation history deleted range
		}
		...
		try {
			...
			const isOpenRouter = this.api instanceof OpenRouterHandler || this.api instanceof CaretHandler
	...
	async presentAssistantMessage() {
		...
		switch (block.type) {
			...
			case "tool_use":
				...
				const askApproval = async (type: CaretAsk, partialMessage?: string) => {
				...
				switch (block.name) {
					...
					case "replace_in_file": {
						...
						try {
							...
							if (diff) {
								...
								// open the editor if not done already.  This is to fix diff error when model provides correct search-replace text but Caret throws error
							...
							const sharedMessageProps: CaretSayTool = {
							...
							if (block.partial) {
								...
								if (this.shouldAutoApproveToolWithPath(block.name, relPath)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Caret wants to ${fileExists ? "edit" : "create"} ${path.basename(relPath)}`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Caret wants to read ${path.basename(absolutePath)}`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Caret wants to view directory ${path.basename(absolutePath)}/`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
									const didApprove = await askApproval("tool", completeMessage)
									if (!didApprove) {
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Caret wants to view source code definitions in ${path.basename(absolutePath)}/`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
									const didApprove = await askApproval("tool", completeMessage)
									if (!didApprove) {
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Caret wants to search files in ${path.basename(absolutePath)}/`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
									const didApprove = await askApproval("tool", completeMessage)
									if (!didApprove) {
							...
							if (block.partial) {
								if (action === "launch") {
									...
									await this.say(
										...
										} satisfies CaretSayBrowserAction),
										undefined,
										block.partial,
									)
								}
					...
					case "execute_command": {
						...
						try {
							if (block.partial) {
								...
								const ignoredFileAttemptedToAccess = this.caretIgnoreController.validateCommand(command)
								if (ignoredFileAttemptedToAccess) {
									await this.say("caretignore_error", ignoredFileAttemptedToAccess)
									pushToolResult(
										formatResponse.toolError(formatResponse.caretIgnoreError(ignoredFileAttemptedToAccess)),
									)
									await this.saveCheckpoint()
									break
								}
					...
					case "use_mcp_tool": {
						...
						try {
							if (block.partial) {
								...
								} satisfies CaretAskUseMcpServer)
								...
								if (mcp_arguments) {
									try {
										...
										await this.say(
											"error",
											`Caret tried to use ${tool_name} with an invalid JSON argument. Retrying...`,
										)
										pushToolResult(
											formatResponse.toolError(
						...
						try {
							if (block.partial) {
								...
								if (optionsRaw && text && parsePartialArrayString(optionsRaw).includes(text)) {
									...
									const lastFollowupMessage = findLast(this.caretMessages, (m) => m.ask === "followup")
						...
						try {
							if (block.partial) {
								...
								if (optionsRaw && text && parsePartialArrayString(optionsRaw).includes(text)) {
									// Valid option selected, don't show user message in UI
									// Update last followup message with selected option
									const lastPlanMessage = findLast(this.caretMessages, (m) => m.ask === "plan_mode_respond")
									if (lastPlanMessage) {
					...
					case "attempt_completion": {
						...
						if (command) {
							...
							// TODO: currently we don't handle if this command fails, it could be useful to let caret know and retry
						...
						*/
						...
						const addNewChangesFlagToLastCompletionResultMessage = async () => {
							...
							const lastCompletionResultMessage = findLast(this.caretMessages, (m) => m.say === "completion_result")
							if (
						...
						try {
							...
							if (block.partial) {
								if (command) {
									...
									// const secondLastMessage = this.caretMessages.at(-2)
	...
	async recursivelyMakeCaretRequests(userContent: UserContent, includeFileDetails: boolean = false): Promise<boolean> {
		if (this.abort) {
			throw new Error("Caret instance aborted")
		}
		...
		if (this.consecutiveMistakeCount >= 3) {
			...
			const { response, text, images } = await this.ask(
				...
				this.api.getModel().id.includes("claude")
					...
					: "Caret uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 3.7 Sonnet for its advanced agentic coding capabilities.",
			)
			if (response === "messageResponse") {
				userContent.push(
					...[
			...
			try {
				this.checkpointTracker = await pTimeout(
					CheckpointTracker.create(this.taskId, this.context.globalStorageUri.fsPath, this.enableCheckpoints),
					{
						milliseconds: 15_000,
						message:
							"Checkpoints taking too long to initialize. Consider re-opening Caret in a project that uses git, or disabling checkpoints.",
					},
				)
			} catch (error) {
				const errorMessage = error instanceof Error ? error.message : "Unknown error"
				console.error("Failed to initialize checkpoint tracker:", errorMessage)
				this.checkpointTrackerErrorMessage = errorMessage // will be displayed right away since we saveCaretMessages next which posts state to webview
			}
		}
		...
		try {
			...
			const abortStream = async (cancelReason: CaretApiReqCancelReason, streamingFailedMessage?: string) => {
				if (this.diffViewProvider.isEditing) {
			...
			try {
				for await (const chunk of stream) {
					...
					if (this.abort) {
						...
						if (!this.abandoned) {
							// only need to gracefully abort if this instance isn't abandoned (sometimes openrouter stream hangs, in which case this would affect future instances of caret)
				...
				// abandoned happens when extension is no longer waiting for the caret instance to finish aborting (error is thrown here when any function in the for loop throws due to this.abort)
				if (!this.abandoned) {
			...
			// OpenRouter/Caret may not return token usage as part of the stream (since it may abort early), so we fetch after the stream is finished
			...
			if (assistantMessage.length > 0) {
				...
				const recDidEndLoop = await this.recursivelyMakeCaretRequests(this.userMessageContent)
	...
	async getEnvironmentDetails(includeFileDetails: boolean = false) {
		...
		// Filter paths through caretIgnoreController
		const allowedOpenTabs = this.caretIgnoreController
		...
		const diagnostics = await this.diagnosticsMonitor.getCurrentDiagnostics(this.didEditFile || terminalWasBusy) // if caret ran a command (ie npm install) or edited the workspace then wait a bit for updated diagnostics
		...
		*/
		...
		if (includeFileDetails) {
			...
			if (isDesktop) {
				...
				const result = formatResponse.formatFilesList(cwd, files, didHitLimit, this.caretIgnoreController)
				details += result
			}
		}
