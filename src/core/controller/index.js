...
import { getAllExtensionState, getGlobalState, getSecret, getWorkspaceState, storeSecret, updateApiConfiguration, updateGlobalState, updateWorkspaceState, } from "../storage/state";
...
export class Controller {
    ...
    constructor(context, outputChannel, postMessage) {
        ...
        this.accountService = new ClineAccountService((msg) => this.postMessageToWebview(msg), async () => {
    ...
    */
    ...
    async initTask(task, images, files, historyItem) {
        ...
        const { apiConfiguration, customInstructions, autoApprovalSettings, browserSettings, chatSettings, shellIntegrationTimeout, terminalReuseEnabled, enableCheckpointsSetting, isNewUser, taskHistory, } = await getAllExtensionState(this.context);
    ...
    async handleWebviewMessage(message) {
        switch (message.type) {
            ...
            case "clearAllTaskHistory": {
                const answer = await vscode.window.showWarningMessage("What would you like to delete?", { modal: true }, "Delete All Except Favorites", "Delete Everything", "Cancel");
    ...
    async togglePlanActModeWithChatSettings(chatSettings, chatContent) {
        ...
        if (shouldSwitchModel) {
            ...
            switch (apiConfiguration.apiProvider) {
                ...
                case "bedrock":
                    await updateWorkspaceState(this.context, "previousModeModelId", apiConfiguration.apiModelId);
                    await updateWorkspaceState(this.context, "previousModeAwsBedrockCustomSelected", apiConfiguration.awsBedrockCustomSelected);
                    await updateWorkspaceState(this.context, "previousModeAwsBedrockCustomModelBaseId", apiConfiguration.awsBedrockCustomModelBaseId);
                    break;
                case "openrouter":
                case "cline":
                ...
                case "vscode-lm":
                    ...
                    await updateWorkspaceState(this.context, "previousModeVsCodeLmModelSelector", apiConfiguration.vsCodeLmModelSelector);
                    break;
                case "openai":
    ...
    async deleteNonFavoriteTaskHistory() {
        ...
        try {
            ...
            vscode.window.showErrorMessage(`Error deleting task history: ${error instanceof Error ? error.message : String(error)}`);
        }
        await this.postStateToWebview();
    }
    ...
    async getStateToPostToWebview() {
        const { apiConfiguration, lastShownAnnouncementId, customInstructions, taskHistory, autoApprovalSettings, browserSettings, chatSettings, userInfo, mcpMarketplaceEnabled, telemetrySetting, planActSeparateModelsSetting, enableCheckpointsSetting, globalClineRulesToggles, globalWorkflowToggles, shellIntegrationTimeout, terminalReuseEnabled, isNewUser, mcpResponsesCollapsed, } = await getAllExtensionState(this.context);
        const localClineRulesToggles = (await getWorkspaceState(this.context, "localClineRulesToggles")) || {};
        const localWindsurfRulesToggles = (await getWorkspaceState(this.context, "localWindsurfRulesToggles")) || {};
        const localCursorRulesToggles = (await getWorkspaceState(this.context, "localCursorRulesToggles")) || {};
        const localWorkflowToggles = (await getWorkspaceState(this.context, "workflowToggles")) || {};
        return {
