...
function assertEquals(actual, expected, message) {
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    console.error(`❌ ${message || 'Assertion failed'}: expected ${JSON.stringify(expected)}, but got ${JSON.stringify(actual)}`);
...
function runTests() {
  ...
  runTest('Todo 항목 삭제 - 존재하는 ID', (mockStorage) => {
    ...
    assertEquals(todoManager.getTodos().find(t => t.id === todoId), undefined, '삭제된 Todo는 찾을 수 없어야 함');
  });
