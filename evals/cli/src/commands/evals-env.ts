...
export async function evalsEnvHandler(options: EvalsEnvOptions): Promise<void> {
	...
	switch (options.action) {
		case "create":
			...
			console.log(chalk.green("The Caret extension should now detect this file and enter test mode."))
		...
		case "remove":
			...
			console.log(chalk.green("The Caret extension should now exit test mode."))
			console.log(chalk.yellow("Note: You may need to reload VSCode for the changes to take effect."))
			break
		...
		case "check":
			...
			if (exists) {
				console.log(chalk.green("The Caret extension should be in test mode."))
			} else {
				console.log(chalk.yellow("The Caret extension should not be in test mode."))
			}
			break
