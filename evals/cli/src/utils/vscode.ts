...
/**
 * Spawn a VSCode instance with the Caret extension
...
export async function spawnVSCode(workspacePath: string, vsixPath?: string): Promise<VSCodeResources> {
	...
	if (!vsixPath) {
		try {
			...
			const caretRoot = path.resolve(process.cwd(), "..", "..")
	...
	const tempUserDataDir = path.join(os.tmpdir(), `vscode-caret-eval-${Date.now()}`)
	...
	const tempExtensionsDir = path.join(os.tmpdir(), `vscode-caret-eval-ext-${Date.now()}`)
	...
	fs.writeFileSync(
		evalsEnvPath,
		`# This file activates Caret test mode
# Created at: ${new Date().toISOString()}
# 
# This file is automatically detected by the Caret extension
...
`,
	...
	// and configure Caret to auto-open on startup
	...
	console.log(`Created settings.json to disable workspace trust and auto-open Caret`)

	// Create keybindings.json to automatically open Caret on startup
	...
	console.log(`Created keybindings.json to help with Caret activation`)
	...
	const args = [
		...
		// Run a command on startup to open Caret
		"--command",
	...
	console.log(`Created startup script to activate Caret`)
	...
	try {
		...
		const extensionDir = path.join(tempExtensionsDir, "caret-activator")
		...
		const packageJson = {
			...
			contributes: {
				commands: [
					{
						command: "caret-activator.activate",
		...
		const extensionJs = `
			...
			function activate(context) {
				console.log('Caret Activator is now active!');
				
				// Register the command to activate Caret
				let disposable = vscode.commands.registerCommand('caret-activator.activate', async function () {
					try {
						// Make sure the Caret extension is activated
						const extension = vscode.extensions.getExtension('saoudrizwan.claude-dev');
						if (!extension) {
							console.error('Caret extension not found');
							return;
						}
						
						if (!extension.isActive) {
							console.log('Activating Caret extension...');
							await extension.activate();
						}
						
						// Show the Caret sidebar
						console.log('Opening Caret sidebar...');
						...
						// Also open Caret in a tab as a fallback
						console.log('Opening Caret in a tab...');
						...
						const caretRootPath = '${path.resolve(process.cwd(), "..", "..")}';
						const visibleWebview = require(path.join(caretRootPath, 'src', 'core', 'webview')).WebviewProvider.getVisibleInstance();
						if (visibleWebview) {
							require(path.join(caretRootPath, 'src', 'services', 'test', 'TestServer')).createTestServer(visibleWebview);
							console.log('Test server created successfully');
						} else {
							console.error('No visible webview instance found');
						}
					} catch (error) {
						console.error('Error activating Caret:', error);
					}
				});
				...
				setTimeout(() => {
					vscode.commands.executeCommand('caret-activator.activate');
				}, 5000);
			}
		...
		console.log(`Created Caret Activator extension`)

		// Try multiple approaches to activate the extension
		let serverStarted = false

		// Create an activation script to run in VS Code
		const activationScriptPath = path.join(settingsDir, "activate-caret.js")
		const activationScript = `
			// This script will be executed to activate Caret and start the test server
			const vscode = require('vscode');
			
			// Execute the caret-activator.activate command
			vscode.commands.executeCommand('caret-activator.activate');
		`
		...
		try {
			console.log("Executing activation script to start Caret and test server...")
			await execa(
				"code",
				[
					"--user-data-dir",
		...
		if (!serverStarted) {
			...
			console.log("You may need to manually open the Caret extension in VS Code")
		}
