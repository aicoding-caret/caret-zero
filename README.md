...
- sLLM(Qwen 2.5 coder / llama 4 Maverick 17B Instruct (128E) ) + RAG 오케스트레이션으로 비용 최적화 
- 토큰 절감 전략
  * Json 기반 시스템 프롬프트 이용 통한 토큰 절감 : [시스템 프롬프트 로딩 성능 비교 보고서](./caret-docs/work-logs/luke-and-alpha/reports/system-prompt-loading-performance.md))
  * 토큰 최적화를 위한 코드 번들링 전송 기술 검토 진행 중 (@[potakim](https://github.com/potakim))  
- 한국어 최적화, 윈도우 대응 강화
- Ceret 자체를 Ceret으로 개발하는 도그푸딩 전략
  * Caret, Windsurf 의 replace_in_file 도구에 대한 버그 개선 (일부 해결 하였으나 근본적으로 정규식을 제거 해야 할 것 같음)
...
- **단기 기술 특징** 
    * 구글의 무료 API지원을 자동으로 이용가능한 옵션 제공
...
- **비용 절감**: Qwen, LLAMA4 등 sLLM 활용 + 캐시 활용 + 프롬프트 구조 개선
...
Ceret은 세 가지 수준의 규칙을 사용하여 에이전트의 행동을 제어합니다:

1.  **Global Rules (글로벌 규칙)**:
    *   **관리 위치**: VS Code 설정의 "Custom Instructions" 섹션에서 직접 관리합니다. (에이전트의 기본 퍼소나 정의)
    *   **참고**: `agents-rules/[에이전트명]/global-rules.json` 파일은 이 설정의 예시 또는 참조용입니다.
2.  **Project Rules (프로젝트 규칙)**:
    *   **관리 위치**: 프로젝트 루트 디렉토리의 `.caretrules` 파일에서 정의합니다. (프로젝트별 특정 지침)
3.  **Mode Rules (모드별 규칙)**:
    *   **관리 위치**: `agents-rules/[에이전트명]/modes.json` 파일 내 각 모드 객체의 `rules` 배열에서 정의합니다. (예: `agents-rules/alpha/modes.json`)
    *   **특징**: 사용자가 이 파일을 수정하여 모드별 행동을 커스터마이징할 수 있으며, 에이전트는 이 변경 사항을 인지하고 적용해야 합니다.

**활용 예시**:

Ceret 에이전트와 대화할 때 다음과 같은 기본 모드를 활용할 수 있습니다 (사용자 정의 가능):
   - `Arch`: 기술 전략 논의, 아키텍처 설계, 업무 범위 설정
   - `Dev`: 페어 코딩 스타일의 협력적 개발 진행
   - `Rule`: 프로젝트(.caretrules) 및 모드(modes.json) 규칙 수정/관리 지원
   - `Talk`: 가벼운 대화 및 사담
   - `Empty`: 사용자가 직접 정의하는 추가 모드

이 계층적 JSON 기반 구조는 토큰 사용량을 최적화하고, 에이전트의 행동을 명확하고 유연하게 정의하여 일관된 개발 경험을 제공합니다.
