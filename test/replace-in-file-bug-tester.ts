...
import { constructNewFileContent } from "../src/core/assistant-message/diff"
// DiffViewProvider 의존성 제거 (DOM 타입 문제 해결)
...
const logger: SimpleLogger = {
	debug: (message: string, data?: any) => console.log(`[DEBUG] ${message}`, data ? data : ""),
	error: (message: string, data?: any) => console.error(`[ERROR] ${message}`, data ? data : ""),
	log: (message: string, data?: any) => console.log(`[LOG] ${message}`, data ? data : ""),
...
async function testReplaceInFile(searchContent: string, replaceContent: string = "", description: string): Promise<boolean> {
	console.log(`\n----- 테스트 케이스: ${description} -----`)
...
^REPLACE^`
		...
		// 1. constructNewFileContent 함수를 사용하여 새 내용 생성 테스트
		...
		// 2. DiffViewProvider 테스트는 건너뛰기 (VS Code 의존성 문제)
...
async function runAllTests() {
	...
	// 각 테스트 케이스 실행
	const testResults: Array<{ name: string; result: boolean }> = []
