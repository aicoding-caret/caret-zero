...
async function* simulateApiCallWithRetry(controller, simulateError = true, errorStatus = 429) {
    if (simulateError) {
        // 에러 응답 시뮬레이션
        const errorBody = errorStatus === 429 
            ? JSON.stringify([
...
async function runTests() {
    ...
    assert(controller1.messages.some(m => m.type === 'retryStatus'), 'retryStatus 메시지가 있어야 함');
    ...
    assert(controller2.messages.some(m => m.type === 'retryStatus' && m.retryState.status === 503), '503 에러 상태가 전송되어야 함');
